...
#define AVDC_FREE(p) free(p)  // "free (void *ptr) deallocates the memory previously allocated by a call to calloc, malloc, or realloc.
 ...
 * 
 * HOW TO/IDEA:
 *  if store each memory block in separate indeces,
 *  i.e., just one memory block is stored at a specific index; then, for having more than "1-way ..."
 *  we will need to have an array where every index points to a line AND in every line there are
 *  more than one cache-line holding data.
 *  So, have to have two values for deciding/chosing cache-line: one for index and another for the right
 *  column at that index.
 */
struct avdc_cache_line {
        avdc_tag_t tag;    // (probably) the tag used for specifying which cache line to target
        int        valid;
        // maybe then need a variable for which column we want the cache line to reside at ... ?
};
...
{
        return pa >> self->tag_shift;     // "pa" is probably "physical address" ... ?
}
...
{
        /* TODO: Update this function */
        // i.e., it needs to flush also all the columns I hopefully have made the prog create, i.e., as 
        // there have to be an array of cache lines
            ...
            avdc_size_t size, avdc_block_size_t block_size, avdc_assoc_t assoc)
{
        ...
        self->block_size_log2 = log2_int32(self->block_size);  // "Log base 2, also known as binary logarithm which is the inverse function of the power of two functions.""
         ...
         avdc_assoc_t assoc)
{
        ...
        if (!avdc_resize(self, size, block_size, assoc)) {   // i.e., if have not avdc.resize(...) then free allocated memory
                AVDC_FREE(self);  // "FROM ABOVE:  #define AVDC_FREE(p) free(p)  // "free (void *ptr) deallocates the memory previously allocated by a call to calloc, malloc, or realloc.
                return NULL;
        }
